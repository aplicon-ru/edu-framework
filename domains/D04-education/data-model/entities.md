...
> Состояние [Запланировано] принадлежит **D14 (расписание)**. D04 получает событие
> уже в [Началось] — инициировано из D14 при старте запланированного занятия.
> Состав передаётся снимком именованного отбора, вычисленного D14 против D05.

```
(D14 порождает) → [Началось] → состав черновой
...
### D14 → D04: запланированное событие → учебное событие

D14 (расписание) является источником учебных событий в D04. При старте запланированного
занятия D14 инициирует создание события в D04 и передаёт:

- тип события (занятие / день практики / аттестационное событие)
- момент начала и длительность
- состав — именованный отбор D14 вычисляет против текущих административных групп D05
  и передаёт D04 **снимком** (D04 его замораживает через буфер)

Статус [Запланировано] принадлежит D14. D04 работает только с фактами реализации.
