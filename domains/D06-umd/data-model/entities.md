...
**Ключевой инвариант источника истины.** РПД **не владеет** трудоёмкостью и формами
контроля — она их **проецирует** из «Строки учебного плана» (read-only, пересобирается при
смене версии плана). Норма-основание содержания — ФГОС; для ДПП — профстандарт/квалтребования.
...
> **Вариант СПО.** Вместо дисциплины — профессиональный модуль / МДК; добавляются
> комплексные формы контроля (квалификационный экзамен). Профиль — `SPO`.
...
| indikator_id | ссылка | Индикатор (§1.9) — единственная точка входа в норму |
...
«строка учебного плана × компетенция/индикатор» **собирается** агрегацией
дескрипторов всех РПД версии ОПОП (`rpd_id → stroka_up_id`, `indikator_id →
kompetenciya_id`), не хранится отдельной M:N-таблицей. Дублирующего источника
истины не возникает: правда лежит в дескрипторах, матрица — их проекция для
чтения (аккредитация, полнота покрытия).
