...
> **Вариант для ДПО.** Дополнительная профессиональная программа (ДПП) — сестринская сущность того же типа, но норма-основание у неё не ФГОС, а составной набор источников (ст. 76; Приказ Минобрнауки от 24.03.2025 № 266). Применимость `[ДПО]`. См. §1.16.
...
содержания — ФГОС; для ДПП — составной `osnovanie` карточки ДПП (§1.16).
...
| fgos_id | ссылка? | ФГОС-основание (`standards/fgos`); для ДПП — пусто, основание см. в `osnovanie` карточки ДПП (§1.16) |
...
> **Вариант ДПО (ДПП).** Норма-основание — составной `osnovanie` карточки ДПП
> (§1.16: квалификационные требования + профстандарт + для переподготовки —
> результаты ФГОС СПО/ВО; ст. 76, Приказ Минобрнауки от 24.03.2025 № 266,
> действует с 01.09.2025 целиком, без отдельной более поздней даты для этого
> требования — исправлено после проверки, было ошибочно указано 01.03.2026).
...
### 1.16 Дополнительная профессиональная программа (ДПП)

Сестринская сущность ОПОП (§1.1) для ДПО: тот же корень-комплект (собственного
содержания не несёт, держит ссылки на версии компонентов), но норма-основание
другое — не ФГОС. Применимость: `[ДПО]`.

**Основание:** ст. 76 ФЗ-273; структура и содержание — Приказ Минобрнауки от
24.03.2025 № 266 «Об утверждении Порядка организации и осуществления
образовательной деятельности по дополнительным профессиональным программам»
(зарегистрирован в Минюсте 22.04.2025 № 81928, действует с 01.09.2025 до
01.09.2031, заменил приказ № 499). ⚠ Ранее в этой карточке была пометка «для
профпереподготовки — с 01.03.2026»: проверено по первоисточнику — неверно, эта
дата нигде не встречается, требование к переподготовке (п. 9 приказа) действует
с той же даты, что и весь приказ целиком, 01.09.2025. Исправлено.

| Атрибут | Тип | Описание |
|---|---|---|
| id | UUID | Внутренний идентификатор |
| vid | enum | повышение_квалификации / профессиональная_переподготовка — не значение, соседствующее с `uroven` ОПОП: структурно разные программы (разный минимум объёма, разный статус квалификации), не уровни одной шкалы |
| naimenovanie | string | Название программы |
| osnovanie | object | Составной набор источников нормы — см. таблицу ниже; не одна ссылка, как `fgos_id` у ОПОП |
| tipovaya_programma_id | ссылка? | Типовая/примерная ДПП, при наличии утверждённая уполномоченным федеральным органом (приказ №266, п. 10) |
| obyom_chasov | number | Объём в академических часах (не з.е.) |
| kvalifikaciya | string? | Присваиваемая квалификация — как правило, отсутствует при повышении квалификации; может присваиваться при переподготовке |
| status | enum | draft / действует / закрыта-для-набора |
| tekushchaya_versiya_id | ссылка | Версия-манифест — по паттерну §1.2 «Версия ОПОП» (та же структура, отдельная линейка версий: `dpp_id`, не `opop_id`) |

**`osnovanie` — набор, не одна ссылка** (приказ №266 требует учитывать
несколько источников одновременно, не выбирать один из них):

| Компонент | Обязательность | Источник |
|---|---|---|
| `kvalifikacionnye_trebovaniya_id` | всегда (п. 7) | Квалификационные требования / ЕКС |
| `profstandart_id` | при наличии профстандарта по направлению (п. 7) | Профессиональный стандарт |
| `fgos_rezultaty_id` | только `vid: профессиональная_переподготовка` (п. 9) | Ссылка на `standards/fgos` — берутся именно «требования к результатам освоения», не структура/объём программы целиком |

**Инварианты.**
- `vid = профессиональная_переподготовка` ⟹ `fgos_rezultaty_id` заполнен, если
  для направления существует применимый ФГОС СПО/ВО (приказ №266, п. 9).
- `obyom_chasov` ≥ 16 при `vid: повышение_квалификации`, ≥ 250 при
  `vid: профессиональная_переподготовка` (приказ №266, п. 18) — жёсткий
  минимум, не рекомендация.
- Если `tipovaya_programma_id` заполнен — содержание ДПП обязано
  соответствовать типовой/примерной программе целиком (приказ №266, п. 10);
  это не аналог «профстандарт как источник одной ПК» (§1.8) — типовая
  программа при наличии обязательна как целое, не как один из источников.

> ⚠ Приказ №266 использует термины «примерная дополнительная профессиональная
> программа» и «типовая дополнительная профессиональная программа» рядом
> (п. 10), не давая ни одному, ни другому отдельного определения — не
> установлено, разные ли это документа (унаследованные из разных норм 273-ФЗ)
> или одно и то же под двумя названиями. Открытый вопрос, не додумываю.

**Не входит в объём этой правки:** параллельные «Учебный план ДПП» и «ФОС
ДПП» — реиспользуют структуру §1.3/§1.7 по аналогии, но полноценно не
специфицированы (закрывался конкретно вопрос нормы-основания, не весь каскад
компонентов ОПОП для ДПП).
...
20. **ДПП (§1.16) — сестринская сущность ОПОП, не её вариант через nullable-поле.**
    `osnovanie` ДПП — составной набор ссылок (квалификационные требования +
    профстандарт + для переподготовки — результаты ФГОС), а не одна ссылка вроде
    `fgos_id` у ОПОП — структурное различие, не просто разный источник одного и
    того же поля (в отличие от УК/ОПК vs ПК, Инвариант 14, где структура полей
    совпадала, различался только держатель).
...
erDiagram
  ...
  DPP }o--o| DPP : "tipovaya_programma_id — при наличии, обязательна к соответствию"
  ...
  DPP {
    enum vid "повышение_квалификации|профессиональная_переподготовка"
    object osnovanie "составной: квалтребования + профстандарт? + fgos_rezultaty (только переподготовка)"
    ref tipovaya_programma_id "опционально; при наличии — обязательна к соответствию"
    number obyom_chasov "минимум 16 (ПК) / 250 (переподготовка) — приказ №266 п.18"
    string kvalifikaciya "как правило пусто при повышении квалификации"
  }
```

> `DPP.tekushchaya_versiya_id` следует паттерну `OPOP_Version` (§1.2), но не
> тем же рядам — отдельная линейка версий (`dpp_id`, не `opop_id`); отдельная
> сущность `DPP_Version` в эту правку не выносилась (см. §1.16, «не входит в
> объём этой правки»).
>
