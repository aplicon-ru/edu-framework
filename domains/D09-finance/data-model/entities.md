...
| Категория обучающихся | Основание льготы (отдельное измерение) | тип, режим применения, модификатор, условие сохранения, срок действия | ← начисление |
...
**Три режима применения льготы (параметр учётной политики):**
- *Снижение цены в договоре* — скидка отражается в приложении к договору; начисление идёт
  по сниженной цене с первого дня.
- *Поверх при расчёте начисления* — цена в договоре базовая; скидка применяется при каждом
  начислении. Гибче при временных льготах.
- *Условное с триггером* — льгота включается при наступлении условия (напр. средний балл
  ≥ 4.5 по итогам сессии → скидка на следующий семестр). Условие сохранения проверяется
  по событию из D04; при несоблюдении скидка снимается новой версией плана.

**Правило комбинирования скидок** (если у обучающегося несколько категорий) —
конфигурируется политикой: суммируются / берётся максимальная / применяется по приоритету
вида. Не настроено явно → «максимальная» как безопасный дефолт.
...
**Цена — в приложении к договору, не в теле.** Тело договора фиксирует услугу, стороны и
общие условия (не меняется). Приложение содержит график платежей по периодам. При изменении
цены (индексация, допсоглашение) оформляется **новое приложение** — тело договора остаётся
неизменным. Это следствие из «версии плана»: v1 = тело + приложение v1; v2 = тело +
приложение v2; слепок шаблона фиксируется один раз при подписании v1.
...
### Три задолженности — одна мета-механика

Доклад §5 (доклад в inbox: `2026-07-06-doklad-digital-infrastructure.pdf`) фиксирует
единство природы:

- **Дисциплинарная** (D04) — из учебных событий внутри дисциплины (пропуск, несданный
  модульный контроль, незащищённая лабораторная)
- **Академическая** (D04) — из аттестационных учебных событий уровня программы
  (экзамен, курсовая, ГИА); работает ФЗ-273 ч. 5 ст. 58
- **Финансовая** (D09, эта сущность) — из административных событий D05 (приказы,
  запускающие оплачиваемый период по договору)

**Одна мета-механика:** событие → обязательство → запись. Задолженность каждого вида —
вычисляемое состояние обязательства (существует и не закрыто положительной записью).

**Разные источники событий:** дисциплинарная и академическая — из **учебных
событий D04**; финансовая — из **административных событий D05** (приказов). Финансовое
обязательство не порождается фактом проведения занятий D04 напрямую — оно порождается
приказом, запускающим оплачиваемый период (зачисление / перевод / условный перевод /
повторное обучение).

Учебный контур поставляет финансовому «оказанный объём услуги» (журнал D04) только
для расчёта возврата при расторжении — не для запуска начислений.
...
> **Терминологическое уточнение.** «Академическое событие» в D09 — это **приказ D05**
> (документное событие: зачисление, перевод, отпуск, отчисление). НЕ путать с
> «учебным событием» D04 (генератор обязательств: занятие, экзамен, модульный
> контроль). Это две разные сущности в двух разных доменах — одинаковое слово
> «событие» используется в двух смыслах. См. D04/GUIDE.md, раздел «Два слоя понятия
> «событие»».
>
> Финансовое обязательство порождается **административным** событием D05 (приказом),
> не **учебным** событием D04. Наступил семестр по приказу о переводе → обязательство
> по договору; сам факт проведения занятий D04 → **оказанный объём услуги** (для
> расчёта возврата при расторжении, D09-P05), но не порождает начислений.
