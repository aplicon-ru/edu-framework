...
Отдельную хранимую «Траекторию» не заводим. Траектория складывается из двух источников:

- **План** — текущий набор → `opop_versiya_id` → структура версии ОПОП (D06: строки
  учебного плана → обязательства D04);
- **Факт** — лог приказов (классификатор D05) + записи и ведомости (D04).

Перевод / смена формы = смена набора приказом → перецепление на другую версию; старые факты
...
ТК РФ ст. 192 (⚠ сверить стык). D04 только поставляет **триггер**: превышен порог
неуважительных пропусков. Сам выговор оформляется и хранится в D05.
